// aby wzorzec byl losowany , 
// aby mozna wybrac tryb rozgrywki na easy np czy wiecej kawelkow cy maj a sie powtorzuc itp 
...
// po co 2 te same petle nie pwotwarzac zbyt rozbudowana powino sie nowe funckje dla emotek
function checkBoard() {
	...
	startBoard();
	drawArrow();
	// to tak samo samoczyne a nie w board
	//Rysuj ostatnia probe
	for ( let i =0; i< 5; i++){
	...
	if(win){
		...
		document.querySelector(".comment").innerHTML = "udalo ci sie zgadzac w rudzie u numerze nr " + round_nr;
		...
		//napisujemy tak nie powino sie roibc  mozna by bylo to zmienic
...
function drawScore() {
	...
	ctx.font = "44px Arial";
	if (round_nr < 10){
		ctx.fillText(round_nr, 680, 105);
	}else{
			ctx.fillText(round_nr, 670, 105);
		}
	ctx.font = "20px Arial";
	ctx.fillText("RUNDA", 650, 165);
}
